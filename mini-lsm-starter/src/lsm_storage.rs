...
impl LsmStorageInner {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        if self.memtable_reaches_capacity_on_put(key, value) {
    ...
    fn memtable_reaches_capacity_on_put(&self, key: &[u8], value: &[u8]) -> bool {
        let size_after_put =
            self.state.read().memtable.approximate_size() + key.len() + value.len();
        size_after_put > self.options.target_sst_size
    }
